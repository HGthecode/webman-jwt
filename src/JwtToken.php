...
use hg\Jwt\Exception\JwtCacheTokenException;
use hg\Jwt\Exception\JwtRefreshTokenExpiredException;
use hg\Jwt\Exception\JwtTokenException;
use hg\Jwt\Exception\JwtConfigException;
use hg\Jwt\Exception\JwtTokenExpiredException;
...
class JwtToken
{
     ...
     * @throws JwtTokenException
     */
    public static function refreshToken(&$_extend = [], ?array $config = null): array
    {
     ...
     */
    public static function generateToken(array $extend, ?array $config = null): array
    {
        self::setConfig($config);
        if (!isset($extend['id'])) {
     ...
     * @author Tinywan(ShaoBo Wan)
     */
    public static function verify(int $tokenType = self::ACCESS_TOKEN, string $token = null, ?array $config = null): array
    {
        self::setConfig($config);
    ...
    private static function _getConfig(): array
    {
        ...
        $config = config('plugin.hg.jwt.app.jwt');
        if (empty($config)) {
     ...
     * @return bool
     */
    public static function clear(string $client = self::TOKEN_CLIENT_WEB, ?array $config = null): bool
    {
        if ($config !== null) {
            self::setConfig($config);
        }
